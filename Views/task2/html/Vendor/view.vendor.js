...
labelEl.textContent = 'Get Vendor Details:';
inputEl.type = 'text';
inputEl.placeholder = 'Enter the Facility Name  ';
inputEl.name = 'EmpId';
...
const getData = async (param = '') => {
  tableEl.innerHTML = '';
  const response = await fetch('http://localhost:3000/vendor' + param);
  if (response.ok) {
...
submitEl.addEventListener('click', () => {
  const param = `/facility=${inputEl.value}/`;
