...
form.setAttribute('action', 'http://localhost:3000/employee/');
...
const updateEmployeeForm = async(id) => {
    ...
    addListElement('number','Postal Code',data.PostalCode,'Your Postal Code','PostalCode',1,0);
    addListElement('text', 'District', data.District, 'District', 1,0);
    addListElement('text','State','Your State',data.State,1,0);
    addListElement('text','Country','Your Country',data.Country,1,0);
    addSelectElement('Is Employee Permanent',3,[`${data.IsPermanent}`,0,1], [data.IsPermanent===0 ? 'No' : 'Yes','No','Yes'],'IsPermanent',1);
