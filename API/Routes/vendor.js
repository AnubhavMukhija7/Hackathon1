...
router.get('/earning/:facilityName/:year', async (req, res) => {
  const result = await getVendorsEarningForFacilityController(
...
router.get('/:id/earning/:year', async (req, res) => {
  const result = await vendorEarningInYearController(
    req.params.id,
    req.params.year
  );
  res.send(result);
...
router.get('/:id/:facility/earning/:year', async (req, res) => {
  const result = await vendorEarningForFacilityInYearController(
    req.params.id,
    req.params.facility,
    req.params.year
  );
